...
In~\secref{avx-anomalies-uncore-freqency}, I describe an anomaly in the uncore frequency selection causing suboptimal energy efficiency.
...
The concept is extended with a new class for AVX512 instructions for the Skylake-SP Microarchitecture~\cite[Sec. 2.6.3]{Intel_Optimization_Reference_Manual_050}.
...
With the addition of the \ac{AMX} instruction set, Intel split up these classes further, ranging from ``Ultra-Light'' to ``Heavy''.
...
The mapping from instruction set and power usage to frequency classes are displayed in~\tabref{avx-classes} based on a slide from Intel published by ServerTheHome~\cite{ServeTheHome_Emerald_Rapids_2023}.
Using a modified version of ``intel-speed-select'', I extract the opportunistic turbo frequencies for each class.
\figref{p0n-frequencies} displays these frequencies for each class and the number of active cores in the class.
...
\begin{figure}[p]
    \centering
    \includegraphics[width=0.6\columnwidth]{fig/avx-frequency-license-bands/avx-frequency-license-bands.pdf}
...
\begin{figure}[bp]
    \centering
    \includegraphics[width=0.6\columnwidth]{fig/avx-frequency-license-bands-validation/validate-avx-frequency-license-bands.pdf}
...
However, two different levels of power draw with the kernels running at the AMX frequency level are visible in~\figref{avx-frequency-uncore-anomaly-package-power-ecdf}.
...
\figref{avx-frequency-uncore-anomaly-max-ee} shows the resulting suboptimal energy efficiency reduction of around \SI{25}{\percent} for the samples with the higher uncore frequency.
...
\begin{figure}[t!]
    ...
    The perfomance is modeled as the number of executed instructions per second by multiplying the freqency times the number of executed instructions per cycle.]{%
    ...
    \figref{avx-frequency-uncore-anomaly-package-power-ecdf} shows the increase in package power for \SI{38}{\percent} of the samples.
    This increase is caused by the increase in the uncore frequency shown in~\figref{avx-frequency-uncore-anomaly-uncore-freq-ecdf}.}
...
I split the measurement data at a interval of \SI{1}{\ms} since the last frequency transition and plot the median latencies in~\figref{pstate_latencies_median_lt_1ms}~and~\figref{pstate_latencies_median_gt_1ms}.
...
To remove the measurement outliers, I chose to display the 95th percentile of transition latencies for all frequency changes in~\figref{pstate_latencies_95percentQuantile}.
Worst case transtion latencies are in the range of \SI{1100}{\us} and \SI{2000}{\us}, depending on the specific frequency change.

To investigate the specifics of the different transition latencies, I plot all the measured transition latencies against the time between detection of last frequency change from source to target frequency.
...
\begin{figure}[]
    ...
    \begin{subfigure}[t]{0.3\linewidth}
        \centering
        \includegraphics[width=\linewidth]{fig/ftalat/ftalat_95percentQuantile_hati.pdf}
        \caption{\label{fig:pstate_latencies_95percentQuantile}95th percentile of P-state switching latencies.}
    \end{subfigure}
    \caption{Core frequency switching latencies of the Sapphire Rappids processor. Median latencies are differentiated by wait times between frequency transitions below and above \SI{1}{\ms}.}
\end{figure}
...
\figref{ufs_init_median} show a median change latency of around \SI{500}{\us} for all frequency transitions.
The worst case latency until this frequency change is under \SI{1000}{\us} as shown in~\figref{ufs_init_95}.
The uncore is blocked up to \SI{5}{\us} as shown in~\figref{ufs_gap_95}.
...
It is an improvement compared to Skylake-SP where Schöne et al. measured \SI{15}{\us}.
...
\figref{c6_latencies} shows a box plot of the exit latency from the C6 idle state.
Outliers are marked in the plot.
The exit latency shows little difference using different core frequencies.
They are more than double of what Schöne et al.~\cite{Schoene_2019_SKL} measured for Skylake-SP.
