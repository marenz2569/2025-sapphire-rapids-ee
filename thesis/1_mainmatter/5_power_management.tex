...
This can be read from the 95th percentile plot in~\figref{pstate_latencies_95percentQuantile}.
...
The higher latency is time dependent, which is shown in~\figref{pstate_latencies_time_dependance}.
We show an example of this higher latency which is also time dependent in~\figref{pstate_latencies_time_dependance}.
For frequency transitions from \SI{1.2}{\GHz} to \SI{1.9}{\GHz}, \SI{2.0}{\GHz}, ... and \SI{2.4}{\GHz}, whilst the transition latency from \SI{1.2}{\GHz} to \SI{1.8}{\GHz} is not influenced by wait or transition times.
...
This can be explain by~\figref{pstate_latencies_time_dependance}, here frequency switches to a higher frequencies have gradually fewer samples that are above a transition latency of around \SI{875}{\us}.
