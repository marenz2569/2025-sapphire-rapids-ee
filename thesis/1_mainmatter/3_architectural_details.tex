...
McCalpin~\cite{McCalpin_2021_CoreMapping} and Cho~\cite{Cho_2022_KnowYourNeighbor} described methods to find the physical location of cores and their associated \ac{CHA}.
...
\figref{physical-layout} shows the physical map of the processors core and \ac{CHA} positions on the test system.
...
Write-allocation evasion and non-temporal store increases the available memory bandwidth to the application by improving cache memory accesses.~\cite{Intel_2020_IceLake_SP}.
Laukemann et al.~\cite{Laukemann_2024_WriteAllocationEvasion} descibe this mechanism in great detail.
...
The changes of key structures between the Skylake-SP and Golden Cove core architecture are displayed in~\tabref{micro-arch-params}.
...
\begin{figure}[]
    ...
    \caption{\label{fig:cbo-latencies-socket-min}Minimal core to core communication latency over all cores of hati's first socket.}
\end{figure}
% \begin{figure}[]
%     \centering
%     \includegraphics[width=0.8\columnwidth]{fig/core-to-core-latency/all-to-all-heatmap-mean.pdf}
%     \caption{\label{fig:cbo-latencies-socket-mean}Average core to core communication latency over all cores of hati's first socket.}
% \end{figure}
\begin{figure}[]
    \centering
    \includegraphics[width=0.8\columnwidth]{fig/core-to-core-latency/all-to-all-heatmap-max.pdf}
    \caption{\label{fig:cbo-latencies-socket-max}Maximal core to core communication latency over all cores of hati's first socket.}
\end{figure}
\begin{figure}[]
    \centering
    \includegraphics[width=0.8\columnwidth]{fig/core-to-core-latency/all-to-all-heatmap-diff.pdf}
    \caption{\label{fig:cbo-latencies-socket-diff}Difference in core to core communication latency over all cores of hati's first socket.}
\end{figure}