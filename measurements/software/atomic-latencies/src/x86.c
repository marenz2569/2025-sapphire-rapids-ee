...
#define TIMER(time)                                                            \
  do {                                                                         \
    /* Macro to get the current time using rdtsc with lfence for               \
     * serialization. */                                                       \
...
#define PING_PONG_CMP_XCHG(address, compare_token, set_token,                  \
                           number_repetitions_rcx, tries_counter)              \
  do {                                                                         \
    register uint64_t _tries asm("r8") = 0;                                    \
    register uint64_t _copy asm("r9") = 0;                                     \
    __asm__ __volatile__(                                                      \
        "movq %%rax, %%r9\n\t" /* Save compare_token (in RAX) to R9 for later  \
                                  reuse */                                     \
        "1:\n\t"               /* Loop label for retries and repetitions */    \
        "incq %%r8\n\t"        /* Increment tries counter (R8) */              \
        "movq %%r9, %%rax\n\t" /* Restore compare_token to RAX before cmpxchg  \
                                */                                             \
        "lock cmpxchg %%rbx, (%%rdi)\n\t" /* Atomically compare RAX with       \
                                             *(RDI); if equal, set *(RDI) =    \
                                             RBX; else, RAX = *(RDI) */        \
        "jnz 1b\n\t"  /* If not zero (cmpxchg failed), retry inner loop */     \
        "loop 1b\n\t" /* Decrement RCX (number_repetitions_rcx); if not zero,  \
                         repeat outer loop */                                  \
        : "+r"(_tries),                                                        \
          "+r"(_copy) /* Output: updated tries and copy registers */           \
...
#define FLUSHADDRESS(address, number_repetitions_rcx)                          \
  do {                                                                         \
    /* Flush the address from the cache using clflush instruction, do a        \
     * clflush in a loop with rcx as a counter and a loop instruction. */      \
    __asm__ __volatile__(                                                      \
        "1:\n\t"              /* Loop label */                                 \
