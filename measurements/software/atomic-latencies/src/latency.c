...
struct function_arguments {
    /* on which CPUs should the threads be placed (first placement allocates and initializes data) */
    ...
    /* an array that will be accessed for flush and cmpxchg measurements, size will be num_cache_lines * cache_line_size */
    char* cache_line_ptr;
...
void* latency_routine(void* args_void) {
    ...
    // while one thread (token == 0) is increasing its responsible location slowly, the other one (token == 1) is doing this all the time. if both are on the same location, we do not measure anything

    if (args->num_thread_locations <= 0 || args->num_cache_lines <= 0 || args->cache_line_size <= 0 || args->cache_line_ptr == NULL) {
    ...
    if ((uintptr_t)args->cache_line_ptr % args->cache_line_size != 0) {
        fprintf(stderr, "Cache line pointer is not aligned to the cache line size.\n");
        return ERROR; // Error code for unaligned cache line pointer
    }

    // parent allocates zeroed memory for latency results (i.e. for each combination of thread_locations, thread_locations and cache lines)
    ...
    if (args->token == 0) {
        ...
        latency_results = malloc(args->num_thread_locations* args->num_thread_locations * args->num_cache_lines * args->num_repetitions * sizeof(uint64_t));
        if (latency_results == NULL){
            fprintf(stderr,"Could not allocate memory for latency results\n");
            return ERROR;
        }

        count_results = malloc(args->num_thread_locations* args->num_thread_locations * args->num_cache_lines * args->num_repetitions * sizeof(uint64_t));
        ...
        flush_results = malloc(args->num_thread_locations* args->num_cache_lines * args->num_repetitions * sizeof(uint64_t));
        ...
        memset(latency_results,0,args->num_thread_locations* args->num_thread_locations * args->num_cache_lines * args->num_repetitions * sizeof(uint64_t));
        memset(count_results,0,args->num_thread_locations* args->num_thread_locations * args->num_cache_lines * args->num_repetitions * sizeof(uint64_t));
        memset(flush_results,0,args->num_thread_locations* args->num_cache_lines * args->num_repetitions * sizeof(uint64_t));
    ...
    // now measure the latency for each combination of thread locations and cache lines
...
//            usleep(10000); // 10 ms
            ...
            for (long long int r = 0; r < args->num_repetitions; r++){
                for (long long int cl = 0; cl < args->num_cache_lines; cl++) {
                    char* cache_line = args->cache_line_ptr + (cl * args->cache_line_size);
                    uint64_t start, stop;
                    TIMER(start);
                    FLUSHADDRESS(cache_line, args->num_iterations);
                    TIMER(stop);
                    flush_results[(i*args->num_repetitions+r)*args->num_cache_lines+cl] = (stop - start);
                }
            }
        }
...
//            usleep(10000); // 10 ms
            ...
            for (long long int r = 0; r < args->num_repetitions; r++) {
                // now we go through the cache lines and repeat the measurements for each cache line
                for (long long int k = 0; k < args->num_cache_lines; k++) {
                    ...
                    uint64_t count = 0;
                    TIMER(start);
                    // now we will use the cache line so that each thread atomically conditionally writes its token to it as soon as the other token is visible
                    PING_PONG_CMP_XCHG(cache_line, other_token, token , args->num_iterations,count);
                    TIMER(stop);
                    if ( token == 0 ) {
                        latency_results[
                            (
                                (i*args->num_thread_locations+j)
                                * args->num_repetitions
                                +r
                            )
                            *args->num_cache_lines
                            +k] = (stop - start);
                        count_results[((i*args->num_thread_locations+j)*args->num_repetitions+r)*args->num_cache_lines+k] = count;
                    }
                }
            }
        }
    ...
    if (args->token == 0) {
        ...
        // write the header
        fprintf(file, "Thread Location 1,Thread Location 2,Repetition,Cache Line,Latency,Repetitions Set,Number Atomic CmpXchg\n");
                ...
                for (int r = 0; r < args->num_repetitions; r++) {
                    for (int k = 0; k < args->num_cache_lines; k++) {
                        fprintf(file, "%lu,%lu,%d,%d,%lu,%lu,%lu\n",
                             args->thread_locations[i], args->thread_locations[j],
                            r,
                            k,
                            latency_results[((i*args->num_thread_locations+j)*args->num_repetitions+r)*args->num_cache_lines+k],
                            args->num_iterations,
                            count_results[((i*args->num_thread_locations+j)*args->num_repetitions+r)*args->num_cache_lines+k]
                        );
                    }
                }
            }
        ...
        // write the header
        fprintf(flush_file, "Thread Location,Repetition,Cache Line,Flush Time,Repetitions Set\n");
        // write the flush results
        for (int i = 0; i < args->num_thread_locations; i++) {
            for (int r = 0; r < args->num_repetitions; r++) {
                for (int j = 0; j < args->num_cache_lines; j++){
                    fprintf(flush_file, "%lu,%d,%d,%lu,%lu\n", args->thread_locations[i], r, j, flush_results[(i*args->num_repetitions+r)*args->num_cache_lines+j], args->num_iterations);
                }
            }
        }
        ...
        fclose(file);
        fclose(flush_file);
        printf("Latency results written to latency_results.txt and flush results written to flush_results.txt\n");
    }
    return NULL; // Indicate successful completion of the thread routine
}

int main(int argc, char *argv[]) {
    // arguments: // 1. locations of threads, 2. number of iterations, 3. number of cache lines, 4. size of cache line, 5. number of repetitions
    // example: ./latency 0,1,5-6 10000 10 128 11
    if (argc < 6) {
        printf("Low-level Latency Measurements  Copyright (C) 2025  TU Dresden, ZIH\n"
                "This program comes with ABSOLUTELY NO WARRANTY; for details type `show w'.\n"
                "This is free software, and you are welcome to redistribute it\n"
                "under certain conditions; type `show c' for details.");
        fprintf(stderr, "Usage: %s <thread_locations> <num_iterations_per_measurements> <number_of_cache_lines> <size_of_cache_line> <num_measurements>\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    // allocat cache lines
    char *cache_lines = aligned_alloc(cache_line_size, num_cache_lines * cache_line_size);
    if (cache_lines == NULL) {
    ...
    struct function_arguments args_parent = {
        .thread_locations = thread_locations_exact,
    ...
    struct function_arguments args_child = {
        .thread_locations = thread_locations_exact,
        .num_thread_locations = num_threads,
        .num_iterations = num_iterations,
        .num_cache_lines = num_cache_lines,
