...
for ((i = 0 ; i < 56 ; i++)); do
    ...
    $FIRESTARTER -b $BINDLIST --measurement --start-delta=15000 --stop-delta=2000 -t 30 -i 6 --run-instruction-groups=REG:100  | tail -n 9 > $OUTFOLDER/lic0/$i.csv
    $FIRESTARTER -b $BINDLIST --measurement --start-delta=15000 --stop-delta=2000 -t 30 -i 6 --run-instruction-groups=REG:100,L1_L:100 | tail -n 9 > $OUTFOLDER/lic1/$i.csv
    $FIRESTARTER -b $BINDLIST --measurement --start-delta=15000 --stop-delta=2000 -t 30 --run-instruction-groups=REG:100 | tail -n 9 > $OUTFOLDER/lic2/$i.csv
    $FIRESTARTER -b $BINDLIST --measurement --start-delta=15000 --stop-delta=2000 -t 30 --run-instruction-groups=L3_L:100 | tail -n 9 > $OUTFOLDER/lic3/$i.csv
done
