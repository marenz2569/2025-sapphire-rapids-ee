...
if [ $scaling_available_frequencies_found -eq 0 ]
then
	export MORE_FLAGS="-DNB_WAIT_RANDOM -DNB_WAIT_US=20000 -DNB_REPORT_TIMES=4000 -DFREQ_SETTER_FILE=\\\"scaling_setspeed\\\""
else
	export MORE_FLAGS="-DNB_WAIT_RANDOM -DNB_WAIT_US=20000 -DNB_REPORT_TIMES=4000 -DFREQ_SETTER_FILE=\\\"scaling_max_freq\\\""
fi
make
cd ..
...
do
	...
	do
		...
		sudo ./ftalat/ftalat $START $TARGET > results/$HOSTNAME/${START}_${TARGET}-out_random_20000us_4000sa.txt
	done
done
